...
void setRGB(int r, int g, int b) {
  // invert PWM for common-anode RGB LED
...
void breathingWhite() {
  breathePhase += 0.15;   // breathing speed

  if (breathePhase > TWO_PI)
      breathePhase = 0;

  // sine breathing curve
  float wave = (sin(breathePhase) + 1.0) / 2.0;

  // eye correction
  wave = pow(wave, 2.2);

  int brightness = 5 + wave * 50;

  ledcWrite(CH_RED,   brightness);
  ledcWrite(CH_GREEN, brightness);
  ledcWrite(CH_BLUE,  brightness);
}
